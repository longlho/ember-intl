...
type MessageFormatValue = Parameters<IntlShape<string>['formatMessage']>[0]['defaultMessage'];
...
export default class FormatMessage extends Formatter<any> {
  ...
  format(locale: string | string[], maybeAst: MessageFormatValue, options?: MessageFormatOptions): string;
  format(
    locale: string | string[],
    maybeAst: MessageFormatValue,
  ...
  format(
    ...
    const result = this.getIntl(locale).formatMessage(
      {
        id: 'dummy',
        defaultMessage: maybeAst,
      },
      escapedOptions,
      { ignoreTag: true }
    );
